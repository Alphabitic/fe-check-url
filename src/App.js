...
import { CancelOutlined, CheckCircleOutlineOutlined, PlayArrowRounded} from '@mui/icons-material';
import "./index.css";
...
<Container>
  <Button onClick={handleCheckLinks} variant="contained" color="primary" startIcon={<PlayArrowRounded />}>
    Vérifier les URLs
  </Button>
  {loading ? (
        ...
        {linkStatuses.map((linkStatus, index) => (
          <tr key={linkStatus.id}>
            ...
            <td style={{ border: '1px solid black', padding: '8px', fontSize: '12pt' }}>
              ...
              {linkStatus.link === 'https://envoludia.neocles.com' && linkStatus.status === 'Lien à vérifier sur bastion' ? (
                <span style={{ fontStyle: 'italic', color: 'gray', fontSize: '12pt' }}> - Lien à vérifier sur bastion</span>
              ) : null}
            </td>
            <td style={{ border: '1px solid black', padding: '8px', fontSize: '12pt' }}>
